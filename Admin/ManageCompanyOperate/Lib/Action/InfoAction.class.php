<?php
     ...
     class InfoAction extends Action
     {
        public function upload()
          {
               ...
               /* $empId=$_SESSION['userid'];*/
	         	...
	         	if (!empty($_FILES)) {
				     ...
				     $targetPath = "../upload/";
                     $titleImgPath=$targetPath."title_".$_FILES['file']['name'];
                     $detailImgPath=$targetPath."detail_".$_FILES['file']['name'];
                
                     $tempFile = $_FILES['file']['tmp_name']; 

                     $titleImgHandler=new ThumbHandler(1);   
                     $titleImgHandler->setSrcImg($tempFile);//设置图片源
                     $titleImgHandler->setMaskImg("../Public/img//shuiyin_logo.gif");
                     $titleImgHandler->setMaskImgPct(10);
                     $titleImgHandler->setCutType(3);//1：自动剪裁   2：手工剪裁 
                     $titleImgHandler->setImgCreateQuality(100);//生成图片质量  数值越大 文件大小也会变大  推荐值100  可根据要求适当改小
                     $titleImgHandler->setImgDisplayQuality(75);//图片显示质量  数值越大 文件大小也会变大  推荐值75-85
                     $titleImgHandler->setDstImg($titleImgPath);//图片生成路径
                     $titleImgHandler->createImg(200,150);
                   //  var_dump($titleImgHandler);
                     

                     $detailImgHandler=new ThumbHandler(0);   
                     $detailImgHandler->setSrcImg($tempFile);//设置图片源
                     $src_info = getimagesize($tempFile);
                     $detailImgHandler->setMaskImg("../Public/img/shuiyin_logo.gif");
                     $detailImgHandler->setMaskImgPct(10);
                     $detailImgHandler->setCutType(3);//1：自动剪裁   2：手工剪裁 
                     $detailImgHandler->setImgCreateQuality(100);//生成图片质量  数值越大 文件大小也会变大  推荐值100  可根据要求适当改小
                     $detailImgHandler->setImgDisplayQuality(85);//图片显示质量  数值越大 文件大小也会变大  推荐值75-85
                     $detailImgHandler->setDstImg($detailImgPath);//图片生成路径
                     $detailImgHandler->createImg(1024,768);
                 //    var_dump($detailImgHandler);
		...
		function deleteFile() 
		{
			  ...
			  $carId=$_POST['carId'];
              $targetPath = "../upload/";
              $titleImgPath=$targetPath."title_".$carId."_".$file;
              $detailImgPath=$targetPath."detail_".$carId."_".$file;
			  unlink($titleImgPath);
              unlink($detailImgPath);
			  echo  $titleImgPath;
		}
...
class ThumbHandler
{
     ...
     * 构造函数
     */
    function ThumbHandler($titleflag)
    {
        $this->mask_font_color = "#ffffff";
        $this->font = 2;
        $this->font_size = 12;
        $this->if_title=$titleflag;
    }
     ...
     * @param    (类型)     (参数名)    (描述)
     */
    function setDstImgBorder($size=0, $color="#000000")
    {
        ...
        if($this->mask_img)
        {
            ...
            if($this->_isFull())
            {
               ...
               // imagesavealpha($this->h_dst, true); 
                $white = ImageColorAllocate($this->h_dst,255,255,255);
                imagefilledrectangle($this->h_dst,0,0,$this->dst_w,$this->dst_h,$white);// 填充背景色
              /*  $trans_colour = imagecolorallocatealpha($this->h_dst, 255, 255, 255, 127); 
                imagefill($this->h_dst, 0, 0, $trans_colour); */
                $this->_drawBorder();
                imagecopyresampled( $this->h_dst, $this->h_src,
                                    $this->start_x, $this->start_y,
                                    $this->src_x, $this->src_y,
    ...
    function _createMaskImg($src)
    {
        ...
        else
        {
                ...
                $this->mask_pos_y=0;
                $w_sep=100;//
                $h_num=($this->dst_h/($this->mask_h+$h_sep));
                $w_num= ($this->dst_w/($this->mask_w+$w_sep));
                for($i=0;$i<$h_num;$i++)
                {
                    $sep=0;
                    if($i%2==0) $sep=80;
                    for ($j=0; $j<$w_num; $j++)
                     { 
                       imagecopymerge($src,
                                    $this->h_mask,
                                    $this->mask_pos_x+$sep+$j*($this->mask_w+$w_sep),$this->mask_pos_y+$h_sep+$i*($this->mask_h+$h_sep),
                                    0, 0,
                                    $this->mask_w, $this->mask_h,
                                    $this->mask_img_pct);
                     }
            ...
            switch($this->cut_type)
            {
                ...
                case 3:
                    ...
                    if($rate_w>$rate_h)
                    {
                         ...
                         $this->setSrcCutPosition(abs($this->src_w-$this->copy_w)/2,0);
                         $this->setRectangleCut($fill_w, $fill_h);
                    }
                    else if($rate_w<$rate_h)
                    {
                         ...
                         $this->setSrcCutPosition(0,abs($this->src_h-$this->copy_h)/2);
                         $this->setRectangleCut($fill_w, $fill_h);
                    }
                    else
                    {      
